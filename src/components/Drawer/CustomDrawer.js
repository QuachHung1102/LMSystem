...
const CustomDrawerContent = (props) => {
  ...
  const navigateTo = useCallback((navigateData, title) => {
    if (navigateData) {
  ...
  const renderItem = useCallback(({ item }) => {
    ...
    return (
      <View>
              ...
              <View style={styles.customItem}>
                <View style={styles.flexRow}>
                  <IconButton
                    ...
                    height={iconSize}
                  />
                  <Text pl2 h3 style={styles.customItemText}>{title}</Text>
                </View>
                {collapse && (
                  <View style={{
                    transform: [collapseStatus ? { rotate: '0deg' } : { rotate: '90deg' }],
                  }}>
                    <IconButton
                      tintColor={colorSet.primaryText}
        ...
        {collapse && (
          <View mh6 pl6>
            <Collapsible collapsed={collapseStatus} style={{ paddingBottom: height * 0.01 }}>
              <AccordionView
                collapse={collapse}
                localized={localized}
  ...
  const DynamicHeader = useCallback(() => {
    return (
      <View style={[{ backgroundColor: colorSet.sixthBackground }, styles.dynamicHeader]}>
  ...
  }, [avatarSize, colorSet.primaryText, colorSet.secondaryBackground, colorSet.sixthBackground, colorSet.thirBackground, statusDotSize, theme.icons.userDefault]);
    ...
    <View style={styles.drawerContent}>
      ...
      <DrawerContentScrollView
        ...
        <FlatList
          ...
          centerContent={false}
          showsVerticalScrollIndicator={false}
          ItemSeparatorComponent={() => <View mh3 style={{
            borderBottomWidth: 1,
            opacity: 0.3,
          }} />}
